...
const RESOURCE_ORIGIN = `${remote.protocol}//${remote.host}`;

// ------------------------- SIMPLE STORAGE -------------------------
const STORE_DIR = join(homedir(), '.mcp-proxy', 'temha');
mkdirSync(STORE_DIR, { recursive: true });
const TOKENS_FILE = join(STORE_DIR, 'tokens.json');
const DISCOVERY_FILE = join(STORE_DIR, 'as-metadata.json');
...
function genPKCE() {
    ...
    return { code_verifier, code_challenge: b64url(challenge) };
}
...
function openInBrowser(url) {
    ...
    try {
        if (platform === 'win32') {
            ...
            spawn(command, [], { stdio: 'ignore', detached: true, shell: true });
...
async function dynamicRegister(as) {
    ...
    if (!as.registration_endpoint) {
        throw new Error('[oidc] no registration endpoint: enable DCR or pre-provision a client');
    }
    ...
    const meta = {
        ...
        token_endpoint_auth_method: reg.token_endpoint_auth_method || (reg.client_secret ? 'client_secret_post' : 'none'),
        redirect_uris: reg.redirect_uris || body.redirect_uris
    };
    writeJSON(CLIENT_FILE, meta);
    return meta;
}

function asBasicAuthHeader(client_id, client_secret) {
...
async function fetchToken(as, client, params) {
    ...
    if (client.client_secret) {
        if (method === 'client_secret_basic') {
            headers['Authorization'] = asBasicAuthHeader(client.client_id, client.client_secret);
        } else if (method === 'client_secret_post') {
            body.set('client_secret', client.client_secret);
        }
    }

    const res = await fetch(as.token_endpoint, { method: 'POST', headers, body });
...
async function ensureClient(as) {
    return await dynamicRegister(as);
}
...
async function ensureToken(as, client) {
    ...
    if (tok && tok.refresh_token) {
        try {
            ...
            console.error('[oauth] refresh failed; interactive login next:', e.message);
        }
    }
...
async function interactiveLogin(conf, client) {
    ...
    const { code } = await new Promise((resolve, reject) => {
        const srv = createServer((req, res) => {
            try {
                ...
                if (!rc || st !== state) {
                    res.writeHead(400).end('OAuth failed');
    ...
    return await fetchToken(conf, client, {
...
async function forwardJsonRpcToRemote(json, accessToken) {
    const res = await fetch(CONFIG.REMOTE_MCP_URL, {
        method: 'POST',
        headers: {
...
async function handleRequest(json) {
    ...
    try {
        return await forwardJsonRpcToRemote(json, tok.access_token);
    } catch (e) {
        if (e.message === 'UNAUTHORIZED') {
            tok = await ensureToken(as, client);
            return await forwardJsonRpcToRemote(json, tok.access_token);
        }
        return { jsonrpc:'2.0', id:json.id ?? null, error:{code:-32001,message:String(e.message)} };
    }
}
...
process.stdin.on('data', async chunk => {
    ...
    while ((i = buf.indexOf('\n')) >= 0) {
        ...
        try {
            const r = await handleRequest(j);
            process.stdout.write(JSON.stringify(r)+'\n');
        } catch (e) {
            process.stdout.write(JSON.stringify({jsonrpc:'2.0',id:j.id??null,error:{code:-32000,message:String(e.message)}})+'\n');
        }
    }
});
